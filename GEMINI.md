...
## 🧭 향후 계획
## 🧭 향후 리팩토링 및 기능 개선 계획 (2025-08-09 최종 수정안)

요청된 여러 기능 추가 및 수정 사항을 바탕으로, 다음과 같이 체계적인 작업 계획을 수립합니다.

### 1단계: 하드웨어 Failsafe와 소프트웨어 제어의 역할 분담 (가장 시급한 아키텍처 변경)
- **목표:** 하드웨어의 독립적인 비상 정지(Failsafe) 기능을 유지 및 강화하고, 이를 백엔드 소프트웨어의 지능적인 판단과 안정적으로 연동시킵니다. "선 조치 후 보고" 원칙을 시스템에 구현합니다.
- **작업 내용:**
    1. **하드웨어 (`arduino/.../safesystem-new.ino`):**
        - **로직 유지:** 적외선 센서 등 치명적인 신호 감지 시, 소프트웨어 명령 없이도 즉시 모터 전원을 차단하는 로직을 유지합니다. (선 조치)
        - **기능 추가:** 비상 정지 발동 후, 백엔드에 "하드웨어 비상 정지 발생"이라는 명확한 시리얼 신호를 전송합니다. (후 보고)
    2. **백엔드 (`control` 계층, `serial_communicator`):**
        - **리스너 수정:** 하드웨어로부터 오는 비상 정지 신호를 감지하고 해석하는 로직을 추가합니다.
        - **상태 동기화:** 비상 신호를 수신하면, 백엔드는 즉시 시스템 상태를 `LOCKED`로 전환하고, 이 사실을 UI 등 다른 계층에 전파합니다.
- **기대 효과:** 소프트웨어의 유연성과 하드웨어의 안정성을 모두 갖춘 이중 안전 시스템을 구축하여, 예측 불가능한 상황에서의 안전성을 극대화합니다.

### 2단계: `LOCK` 기능 구현 (핵심 안전 기능)
- **목표:** '사람 넘어짐'이나 '비상 센서 감지' 등 심각한 위험 상황 발생 시, 관리자의 수동 리셋 없이는 시스템이 재가동되지 않는 '잠금(LOCKED)' 상태를 구현합니다.
- **작업 내용:**
    1. `state_manager`에 `LOCKED` 상태 추가 및 상태 관리 로직 구현.
    2. `rule_engine`이 특정 위험에 대해 `LOCK_SYSTEM` 액션을 생성하도록 수정.
    3. `vision_worker`가 `LOCK_SYSTEM` 및 `RESET` 명령을 처리하도록 수정.
    4. `control_api`에 `/reset` 엔드포인트 추가.
    5. 프론트엔드에 `LOCKED` 상태를 명확히 표시하고, '리셋' 버튼 및 관련 로직 추가.
- **기대 효과:** 시스템의 안전 등급을 대폭 향상시킵니다.

### 3단계: 경보(Alarm) 시스템 상세 구현
- **목표:** 위험 종류에 따라 경보의 수준과 동작 방식을 체계적으로 정의하고 구현합니다.
- **작업 내용:**
    1. `rule_engine`에서 위험 상황별로 `CRITICAL`, `HIGH`, `MEDIUM` 등급의 `TRIGGER_ALARM` 및 `STOP_ALARM` 액션을 생성하도록 규칙을 상세히 구현.
    2. `CRITICAL` 경보는 `LOCKED` 상태와 연동하여, 오직 '리셋' 명령으로만 해제되도록 구현.
- **기대 효과:** 사용자에게 위험 수준에 대한 명확하고 직관적인 피드백을 제공합니다.

### 4단계: 제어 로직 전체 검증
- **목표:** 1~3단계에서 변경된 모든 제어 로직의 전체 흐름을 검토하고, 잠재적인 충돌이나 예외 상황을 찾아 수정합니다.
- **기대 효과:** 시스템 안정성 및 신뢰성 확보.

### 5단계: 코드 전체 리팩토링
- **목표:** 전체 코드베이스의 일관성과 가독성을 향상시킵니다.
- **작업 내용:**
    1. **이름 획일화:** 변수, 함수, 컴포넌트, CSS 클래스 등의 작명 규칙을 통일합니다.
    2. **색상 통일:** UI에서 사용되는 상태(안전, 경고, 위험) 표시 색상을 변수화하고 통일합니다.
- **기대 효과:** 장기적인 유지보수 비용을 절감합니다.
---
