...
## UI/UX 구성
1. 사용자 인증 (로그인 및 회원가입)
   - 기존 사용자는 ID와 비밀번호로 로그인하여 메인 화면으로 진입
   - 계정이 없는 신규 사용자를 위해, 별도의 회원가입 보드를 통해 계정을 생성할 수 있는 기능을 제공
   <img width="785" height="530" alt="image" src="https://github.com/user-attachments/assets/65804ed4-7355-4d30-a7e4-70c6ba043e87" />


2. 대시보드 초기 화면
   - 로그인 후 먼저 접하는 메인 화면
   - 좌측 (실시간 모니터링 영역): 화면의 좌측에는 현장의 CCTV 영상이 실시간으로 송출되어, 시각적인 현장 상황 감시가 이루어짐
   - 우측 (정보 및 제어 영역): 시스템의 상태를 제어하는 '컨트롤 보드'와 과거 이벤트 이력을 보여주는 '영상 로그'가 배치되어, 현재 시스템의 논리적 상태와 과거 기록을 동시에 확인 가능
   <img width="1082" height="507" alt="image" src="https://github.com/user-attachments/assets/538dac67-5d3f-421b-ae19-1c80cb78ebb3" />

3. 위험 구역 설정
    - 관리자는 '위험 구역 설정' 모드를 통해, 마우스 클릭만으로 영상 화면 위에 AI가 감시할 가상의 위험구역을 직접 설치하고 관리할 수 있음
   <img width="979" height="308" alt="image" src="https://github.com/user-attachments/assets/b908f0f4-f998-4f33-acce-04e69936864b" />
   <img width="1089" height="499" alt="image" src="https://github.com/user-attachments/assets/4d29cc82-4363-4360-bd52-43190cff32f3" />


4. 컨베이어 작업 모드 제어
   - 우측 '컨트롤 보드' 패널을 통해 시스템의 상태를 제어하고, 결과를 영상에서 즉시 확인 가능
   <img width="405" height="315" alt="image" src="https://github.com/user-attachments/assets/c7d7ec57-35c4-4e29-a23c-bf8eab627104" />

5. 위험 경고 알림
   - LOTO 시스템<br>
     → 디지털 LOTO(Lock Out, Tag Out) 기능이 활성화된 ‘정비 모드’에서는 작업자의 안전을 보장하기 위해, 시스템이 사람의 존재를 인지하는 한 어떠한 재가동 시도도 거부하도록 설계<br><br>
     → 작업자가 위험 구역 내에 머무르고 있는 상태에서는, 설령 다른 관리자가 실수로 ‘운전 모드’로 전환을 시도하더라도 시스템은 이 명령을 무시<br>
   <img width="1062" height="523" alt="image" src="https://github.com/user-attachments/assets/2ab37052-a841-40b7-943c-cb5ef04d3121" /><br>

   - LOCK 시스템<br>
     → 넘어짐 또는 적외선 센서가 감지되면 우측 컨트롤 화면에 붉은색의 경고 알림창을 띄워 관리자의 주의를 즉시 환기시킴<br><br>
     → 전체 시스템이 잠그고, 관리자가 현장을 확인한 후 시스템 리셋 버튼을 눌러야 시스템을 재가동 가능<br>
   <img width="1091" height="522" alt="image" src="https://github.com/user-attachments/assets/6adccee8-0419-4151-bc7c-6ee7cbae3c78" />


6. 비디오 로그 기록
    - 시스템의 모든 이벤트는 우측 영상 로그에 자동으로 기록되어, 체계적인 이력 관리를 지원
    <img width="625" height="407" alt="image" src="https://github.com/user-attachments/assets/1ac68f66-2f0a-4f14-83c1-f4e5b52d1caf" />
